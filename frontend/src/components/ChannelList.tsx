  ...
  return (
    <div className="w-56 flex flex-col bg-surface-800 border-r border-surface-700">
      <div className="px-4 py-3 border-b border-surface-700 flex items-center justify-between">
        <span className="font-semibold text-ivory text-sm truncate">{serverName}</span>
      </div>
      ...
      <div className="flex-1 overflow-y-auto scrollbar-thin py-2">
        <div className="px-3 mb-1 flex items-center justify-between">
          ...
          <button
            ...
            className="text-ivory/50 hover:text-ivory text-lg leading-none"
          >
            +
          </button>
        </div>
          ...
          <button
            ...
            className={`w-full text-left px-3 py-1.5 rounded-md mx-1 flex items-center gap-1.5 text-sm transition-colors
              ${selectedChannelId === c.id
                ? 'bg-surface-600 text-ivory'
                : 'text-ivory/60 hover:text-ivory/90 hover:bg-surface-700'
              }`}
          >
